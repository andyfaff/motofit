...
Function Setupmultilayer(ctrlname,checked) : CheckboxControl
	...
	switch(checked)	
		case 0:
			...
			break						
		case 1:		
			Variable/g root:packages:motofit:reflectivity:tempwaves:Vmullayers=str2num(moto_Str("Vmullayers"))
			Variable/g root:packages:motofit:reflectivity:tempwaves:Vmulrep=str2num(moto_Str("mulrep"))
			Variable/g root:packages:motofit:reflectivity:tempwaves:Vappendlayer=str2num(moto_Str("mulappend"))
...
//this function adds and removes controls for the multilayer panel.
	SVAR/Z Motofitcontrol=root:packages:motofitcontrol
	if(varnum!=999)
...
//if there are more layers add controls
	...
	elseif(nlayers>oldlayers)						//make the new waves
		...
		do		//ii is the layer number
			...
			partitle=num2istr(parindex+1)+". iSLD"+num2istr(ii+1)				
			...
			Popupmenu $controlname,pos={660,ypos-3},mode=1,fsize=6,popvalue="_none_",value=#"root:packages:motofit:reflectivity:SLDdatabase:chemicals",proc=Moto_SLDintocoef_Cref,win=multilayerpanel
...
Function BuildMultilayerpanel() : Panel
	...
	SetVariable mullayers,limits={0,Inf,1},value= root:packages:motofit:reflectivity:tempwaves:Vmullayers
	SetVariable mulrep,pos={2,25},size={194,16},proc=Motofit_Varproc,title="Number of repetitions"
	SetVariable mulrep,limits={0,Inf,1},value= root:packages:motofit:reflectivity:tempwaves:Vmulrep
	SetVariable mulAppend,pos={2,51},size={194,16},proc=Motofit_Varproc,title="Append to stack to layer No."
	SetVariable mulAppend,limits={0,Inf,1},value= root:packages:motofit:reflectivity:tempwaves:Vappendlayer
...
//this will take coef_multiCref and turn it back into coef_Cref and Multilayer
	...
	Variable parbase=coef_Cref[0]*4+8,parmulti=numpnts(coef_multiCref)-parbase
	Variable ii=1
	do
...
//the first layer is the top layer
	...
	if(abs(numpnts(multikernel)-totallength)>=0)
		...
		Multikernel[0]=(numpnts(multikernel)-8)/4
	endif

	for(ii=0;ii<Vappendlayer;ii+=1)	
		Multikernel[4*(ii+1)+4]=coef_multiCref[4*(ii+1)+4]		
		Multikernel[4*(ii+1)+4+1]=coef_multiCref[4*(ii+1)+4+1]
		Multikernel[4*(ii+1)+4+2]=coef_multiCref[4*(ii+1)+4+2]
		Multikernel[4*(ii+1)+4+3]=coef_multiCref[4*(ii+1)+4+3]						
	endfor											
...
//within the multilayer
	...
	for(mm=0;mm<Vmulrep;mm+=1)
		if(mm==0)
			redimension/n=(8+(4*ii)) Multikernel
		endif
		concatenate/NP {repeat},multikernel	
		ii+=numpnts(repeat)/4
	endfor
	...
	for(mm=Vappendlayer+1;mm<coef_Cref[0]+1;mm+=1)	
		Multikernel[4*ii+4]=coef_multiCref[4*mm+4]		
		Multikernel[4*ii+4+1]=coef_multiCref[4*mm+4+1]
		Multikernel[4*ii+4+2]=coef_multiCref[4*mm+4+2]
		Multikernel[4*ii+4+3]=coef_multiCref[4*mm+4+3]
		ii+=1								
	endfor

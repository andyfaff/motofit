...
Function FitRefToListOfWaves()
	...
	if(cmpstr(S_Value,"Genetic")==0 || cmpstr(S_Value,"Genetic + LM")==0 )
		GEN_setlimitsforGENcurvefit(root:packages:motofit:MOTOFIT_batchfits:temporaryparameterwave,holdstring,"root:packages:motofit:MOTOFIT_batchfits")
	endif
...
//		duplicate/o $aWaveName_X , root:packages:motofit:reflectivity:tempwaves:temp_dq
//		duplicate/o $aWaveName_X , temp_dq
//		Wave temp_dq=root:packages:motofit:GEN_optimise:temp_dq
//		REQUIRED: make a functional form for dq
		...
		case "Genetic":
			controlinfo/w=reflectivitypanel useerrors
			if(V_Value)
				cmd="gencurvefit "+errors +"/q/n/X="+awavename_x+"/D="+fitdestination+ " motofit,"+awavename_y +",root:packages:motofit:Motofit_batchfits:temporaryparameterwave,\""+holdstring+"\",root:packages:motofit:old_genoptimise:GENcurvefitlimits"
				Execute/Z cmd
				duplicate/o $awavename_x,$("fitx_"+awavename_x)
			else
				cmd="gencurvefit /q/n/X="+awavename_x+"/D="+fitdestination+ " motofit,"+awavename_y +",root:packages:motofit:Motofit_batchfits:temporaryparameterwave,\""+holdstring+"\",root:packages:motofit:old_genoptimise:GENcurvefitlimits"		
				Execute cmd
				duplicate/o $awavename_x,$("fitx_"+awavename_x)
			endif
		break
		case "Levenberg-Marquardt":
			cmd="FuncFit/H=\""+holdstring+"\"/N /Q Motofit root:packages:motofit:Motofit_batchfits:temporaryparameterwave "+aWaveName_Y+"/X="+aWaveName_X+" /D="+fitdestination+constraint+errors
			Execute/Z cmd
		break
		case "Genetic + LM":
			controlinfo/w=reflectivitypanel useerrors
	...
	//	moto_repstr("dataset",dataset)
		...
		concatenate {root:packages:motofit:motofit_batchfits:temporaryparameterwave},root:packages:motofit:MOTOFIT_batchfits:concat_coef

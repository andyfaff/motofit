...
Menu "Motofit"
	...
	Submenu "reduction"
		...
		"reduce neutrons", reduceneutrons()
	end
...
Function plotCalcref()
	...
	newdatafolder/o root:motofit:reflectivity:tempwaves
	...
	String/G root:motofit:reflectivity:Motofitcontrol
	SVAR/z motofitcontrol = root:motofit:reflectivity:Motofitcontrol
	...
	make/o/d/n=10 coef_Cref={1,1,0,2.07,1e-5,4,25,3.47,0,4}
	...
	//although they don't have to be displayed coz of the panel, I am so that they can be pasted
...
Function Moto_autoscaleRef(B_Struct)
	STRUCT WMButtonAction &B_Struct
...
Function Moto_update()
	...
	Setdatafolder root:
	Wave coef_Cref,theoretical_R,theoretical_q,multikernel,multilay,SLD,zed,coef_multiCref
...
Function Moto_createmotofitwaves(w)
	...
	newdatafolder/o/s root:motofit:reflectivity
	newdatafolder/o/s root:motofit:reflectivity:tempwaves
...
Function Motofit(w,y,z) :Fitfunc
	...
	if(datafolderexists("root:motofit:reflectivity:tempwaves")==0)
	...
	//if an external XOP exists to do the calculation, then use that, as it'll be 
	//~x5 faster.
	String ABXOP=(Functionlist("Abeles*",";","KIND=4"))
	variable allatonce=0
	if(stringmatch(ABXOP,"*AbelesAll*"))
		allatonce = 1
		ABXOP = "AbelesAll"	
		Funcref GEN_allatoncefitfunc fall = $ABXOP
	elseif(Stringmatch(ABXOP,"*Abeles*"))
		ABXOP = "Abeles"
		Funcref GEN_fitfunc f = $ABXOP
	else
		allatonce=1
		ABXOP = "Moto_AbelesReflectivity"
		Funcref GEN_allatoncefitfunc fall = $ABXOP
		Moto_createmotofitwaves(w) 
	endif
	...
	setdatafolder root:motofit:reflectivity:tempwaves
	switch(usedqwave)
		case 0:					//case 0 is if usedqwave=0.  This means the user hasn't selected a dQ wave
			...
			if (res<0.005)		//if the resolution is less that 1% don't bother doing the convolution
				...
				Make/o/d/n=(gaussnum) root:motofit:reflectivity:tempwaves:gausswave
				Wave gausswave = root:motofit:reflectivity:tempwaves:gausswave
				Setscale/I x,-res,res,gausswave
				Gausswave=gauss(x,0,res/2)
				...
				Variable logspace=log(lowQ*(1+res))-log(lowQ)
				Variable start=log(lowQ)-8*res
				Variable finish=log(highQ*(1+8*res))
				Variable interpnum=round(abs(2*(abs(start-finish))/(res/gaussgpoint)))
				make/o/d/n=(interpnum) root:motofit:reflectivity:tempwaves:logxtemp
				make/o/d/n=(interpnum) root:motofit:reflectivity:tempwaves:ytemp
				make/o/d/n=(interpnum) root:motofit:reflectivity:tempwaves:xtemp
				Wave logxtemp = root:motofit:reflectivity:tempwaves:logxtemp
				Wave xtemp = root:motofit:reflectivity:tempwaves:xtemp
				Wave ytemp = root:motofit:reflectivity:tempwaves:ytemp
				...
				variable number2d=round(6*res/((abs(start-finish))/(interpnum)))-1
				...
				variable gaussum=1/(sum(gausswave,-res,res))
				fastop ytemp=(gaussum)*ytemp
				duplicate/o z, root:motofit:reflectivity:tempwaves:xrtemp
				duplicate/o z, root:motofit:reflectivity:tempwaves:ytemp2
				Wave ytemp2 = root:motofit:reflectivity:tempwaves:ytemp2, xrtemp =  root:motofit:reflectivity:tempwaves:xrtemp
		...
		case 1:			//if usedqwave=1 then this means the user has selected his own resolution wave (requires 4 column data) 
			if(!allatonce)
				y=f(w,z)
				y += 0.056*f(w,z+1.2*dq)
				y += 0.056*f(w,z-1.2*dq)
				y+=0.135*f(w,z-1.0*dq)
				y+=0.135*f(w,z+1.0*dq)
				y+=0.278*f(w,z-0.8*dq)
				y+=0.278*f(w,z+0.8*dq)
				y+=0.487*f(w,z-0.6*dq)
				y+=0.487*f(w,z+0.6*dq)
				y+=0.726*f(w,z-0.4*dq)
				y+=0.726*f(w,z+0.4*dq)
				y+=0.923*f(w,z-0.2*dq)
				y+=0.923*f(w,z+0.2*dq)
				y /= 6.211	
			elseif(allatonce)
				setdatafolder root:motofit:reflectivity:tempwaves
				//need to implement all at once version for point-by-point smearing.  This is if the user doesn't 
				//have the new Abeles XOP
				duplicate/o z,root:motofit:reflectivity:tempwaves:ytemp
				duplicate/o z,root:motofit:reflectivity:tempwaves:xtemp
				Wave xtemp = root:motofit:reflectivity:tempwaves:xtemp, ytemp = root:motofit:reflectivity:tempwaves:ytemp
				y=0
				fastop xtemp = z-1.2*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.056*ytemp
				fastop xtemp = z+1.2*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.056*ytemp
				fastop xtemp = z-1.0*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.135*ytemp
				fastop xtemp = z+1.0*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.135*ytemp
				fastop xtemp = z-0.8*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.278*ytemp
				fastop xtemp = z+0.8*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.278*ytemp
				fastop xtemp = z-0.6*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.487*ytemp
				fastop xtemp = z+0.6*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.487*ytemp
				fastop xtemp = z+0.4*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.726*ytemp
				fastop xtemp = z-0.4*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.726*ytemp
				fastop xtemp = z-0.2*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.923*ytemp
				fastop xtemp = z+0.2*dq
				fall(w,ytemp,xtemp)
				fastop y=y+0.923*ytemp
				fall(w,ytemp,z)
...
Function Moto_Plotreflectivity()
	...
	String Q,R,dR,dQ
	Q = CleanupName((S_fileName + "q"),0)
	R = CleanupName((S_fileName + "R"),0)
	dR = CleanupName((S_fileName + "E"),0)
	dQ = CleanupName((S_fileName + "dq"),0)
	
	//if you're not loading 2,3 or 4 column data then there may be something wrong.
	if(V_Flag<2 || V_Flag>4)
	...
	//if you've already loaded that file then a wave with name $Q will exist, so you can't go further with
	//the loading
	if(waveexists($Q) !=0)
		Killwaves/z wave0,wave1,wave2,wave3
		Abort "This file has already been loaded. Use Append to Graph..."
	endif
...
Function Moto_Reflectivitypanel() : Panel
	...
	ListBox baseparams,listWave=root:motofit:reflectivity:baselayerparams
	ListBox baseparams,selWave=root:motofit:reflectivity:baselayerparams_selwave
	ListBox baseparams,mode= 6,editStyle= 1,widths={19,89,21}
	
	ListBox layerparams,pos={70,289},size={564,133},proc=moto_modelchange
	ListBox layerparams,listWave=root:motofit:reflectivity:layerparams
	ListBox layerparams,selWave=root:motofit:reflectivity:layerparams_selwave
	...
	Setvariable FThighQ title = "high Q for FFT",bodyWidth=60,value=root:motofit:reflectivity:tempwaves:FThiQ
	Setvariable FThighQ,pos={160,496},proc=Moto_FTreflectivity,limits={0.005,1,0.01}
	Setvariable fringe,pos={148,543},size={166,15},proc=Moto_fringespacing,title="layer thickness spacing"
	Setvariable fringe,limits={0,0,0},barmisc={0,1000},value= root:motofit:reflectivity:tempwaves:fringe
	...
	TabControl foo,pos={7,2},size={716,633},proc=Moto_fooProc,tabLabel(0)="Fit tab"
	TabControl foo,tabLabel(1)="Constraints tab",tabLabel(2)="Make globals tab"
	...
	string cmd="ValDisplay Chisquare title=\"Chi squared\",pos={252,230},size={170,15},value=root:motofit:reflectivity:chisq"
	Execute/Q/Z cmd
...
Function moto_thicknesstabProc(TC_Struct)
	...
	tabcontrol thicknesstab,value=tab,win=Reflectivitypanel
	...
	if(tab==0 && Waveexists(root:motofit:reflectivity:tempwaves:FFToutput))
		...
		if(stringmatch(Childwindowlist("ReflectivityPanel"),"*FFTplot*")==1)
			variable ii=0
			string subwindow=listmatch(Childwindowlist("ReflectivityPanel"),"FFTplot*",";"),tokill
			for(ii=0;ii<itemsinlist(subwindow);ii+=1)
				tokill = stringfromlist(ii,subwindow)
				Killwindow reflectivitypanel#$tokill
			endfor
		endif
	endif
...
Function moto_fooProc(TC_Struct)
	...
	if(waveexists(root:motofit:reflectivity:constraints))
		do
			conname="constraint"+num2istr(ii)
			Setvariable $conname,disable=(tab!=1)
			ii+=1
		while(ii<numpnts(root:motofit:reflectivity:constraints))
	endif
...
Function Moto_Chi2_print()
	...
	duplicate/o $dataset_y, root:motofit:reflectivity:tempwaves:sim_y
	duplicate/o $dataset_y, root:motofit:reflectivity:tempwaves:enum
	duplicate/o $dataset_x,root:motofit:reflectivity:tempwaves:sim_x
	duplicate/o $dataset_y,root:motofit:reflectivity:tempwaves:sim_e
	
	Wave sim_y = root:motofit:reflectivity:tempwaves:sim_y
	Wave sim_x = root:motofit:reflectivity:tempwaves:sim_x
	Wave sim_e = root:motofit:reflectivity:tempwaves:sim_e
	Wave enum = root:motofit:reflectivity:tempwaves:enum
	...
	Duplicate/o coefwave,root:motofit:reflectivity:tempwaves:holdwave
	Wave holdwave = root:motofit:reflectivity:tempwaves:holdwave
	holdwave=1
	//update the Chisquared display
	Variable/g root:motofit:reflectivity:chisq=moto_chi2(holdwave,coefwave)/numpnts(sim_y)
End
...
Function moto_chi2(xx,pp)
	...
	Wave/z sim_y = root:motofit:reflectivity:tempwaves:sim_y
	Wave/z sim_x = root:motofit:reflectivity:tempwaves:sim_x
	Wave/z	sim_e = root:motofit:reflectivity:tempwaves:sim_e
	Wave/z enum = root:motofit:reflectivity:tempwaves:enum
...
Function Moto_FitDataButton(B_Struct)
	STRUCT WMButtonAction &B_Struct
	if(B_Struct.eventcode!=2)
		return 0
	endif
...
Function Moto_fit_Genetic()
	...
	y=cleanupname(dataset+"R",0)
	x=cleanupname(dataset+"q",0)
	e=cleanupname(dataset+"E",0)
	dx=cleanupname(dataset+"dq",0)
...
Function Moto_parse_equalconstraints(constraints)
	...
	String constantfolder="root:motofit:reflectivity"
...
Function Moto_loaddata()
	...
	Moto_Loaddatas(B_Struct)
End

Function Moto_Loaddatas(B_Struct)
	STRUCT WMButtonAction &B_Struct
...
Function Moto_Makeglobals(ctrlName) : ButtonControl
	...
	if(strlen(test)==0)
		Killdatafolder/Z root:MOTOFITGF:WindowCoordinates
...
Function Moto_changecoefs(PU_Struct)
	...
	if(multilayer==1)
		...
		Variable/G root:motofit:reflectivity:tempwaves:Vmullayers=numpnts(multilay)/4
		NVAR/z Vmullayers = root:motofit:reflectivity:tempwaves:Vmullayers
...
Function Moto_loadcoefwave(ctrlName) : ButtonControl
	String ctrlName
	//this function loads in a previously saved coefficient wave
	//the user should save the coefficient waves in the same manner that IGOR created them during the fit.
	SVAR/Z Motofitcontrol=root:motofit:reflectivity:motofitcontrol
...
Function Moto_fringespacing(ctrlName,nofringes,varStr,varName) : SetVariableControl
	...
	NVAR/Z fringe = root:motofit:Reflectivity:tempwaves:fringe
	Variable delq
	if (WaveExists(CsrWaveRef(A)) %& WaveExists(CsrWaveRef(B)))
		if (CmpStr(CsrWave(A),CsrWave(B)) != 0)
			abort "The cursors are not on the same wave. Please move them so that they are."
...
Function Moto_cursorleft(ctrlname) :Buttoncontrol
	...
	if(cmpstr(ctrlname,"cursorleftA")==0)
		cursor/A=1 A,$cursors,(a-1)
	endif
	if(cmpstr(ctrlname,"cursorleftB")==0)
		cursor/A=1 B,$cursors,(b-1)
	endif
	Doupdate
End
...
Function Moto_FTreflectivity(SV_Struct)
	STRUCT WMSetVariableAction &SV_Struct
	...
	Duplicate/o y root:motofit:reflectivity:tempwaves:tempy
	Duplicate/o x root:motofit:reflectivity:tempwaves:tempx
	Setdatafolder root:motofit:reflectivity:tempwaves
	
	Wave/z tempy,tempx,dummy
	sort tempx,tempx,tempy
	
	NVAR/z FThiQ = root:motofit:Reflectivity:tempwaves:FThiQ
	NVAR/z FTlowQ = root:motofit:Reflectivity:tempwaves:FTlowQ
	variable level,start,finish

	if(NVAR_exists(FTlowQ)!=1)
		variable/g root:motofit:Reflectivity:tempwaves:FTlowQ
		NVAR/z FTlowQ = root:motofit:Reflectivity:tempwaves:FTlowQ
	...
	if(NVAR_exists(FThiQ)!=1)
		variable /g root:motofit:Reflectivity:tempwaves:FThiQ
		NVAR/z FThiQ = root:motofit:Reflectivity:tempwaves:FThiQ
	...
	FFT/pad={8*numpnts(FFTwave)}/dest=FFTwave/winf=cos1 FFTwave
	make/o/d/n=(numpnts(FFTWave)) FFToutput
	FFToutput=cabs(FFTwave)
	Variable x2=pnt2x(FFTwave,2)
	Deletepoints 0,2,FFToutput
	Setscale/P x,2*Pi*x2,deltax(FFTwave)*2*Pi,FFToutput
...
Function Moto_changeconstraint(ctrlname): ButtonControl
	...
	if(cmpstr(ctrlname,"addconstraint")==0)
		if(waveexists(root:motofit:reflectivity:Constraints)==0)
			Make/T/n=1 root:motofit:reflectivity:constraints
			SetVariable constraint0,pos={311,61},size={236,16},title="Constraint0",value= root:motofit:reflectivity:constraints[0]
		else
			conlength=numpnts(root:motofit:reflectivity:constraints)
			redimension/n=(conlength+1) root:motofit:reflectivity:constraints
			conname="constraint"+num2istr(conlength)
			Variable conpos=61+(20*conlength)
			SetVariable $conname,pos={311,conpos},size={236,16},title=conname,value= root:motofit:reflectivity:constraints[conlength]
		endif
	endif
	...
	if(cmpstr(ctrlname,"removeconstraint")==0)
		...
		if(conlength==1)
			killwaves/Z root:motofit:reflectivity:constraints
...
Function Moto_add_chemical(ctrlName) : ButtonControl
	...
	string saveDF = getdatafolder(1)
	Setdatafolder root:motofit:reflectivity:SLDdatabase
	String/G chemicals
...
Function Moto_SLDdatabase() : Panel
	...
	if(strlen(winexist)>0)
		...
		if(datafolderexists("root:motofit:reflectivity:SLDdatabase")==0)
			newdatafolder/o root:motofit
			newdatafolder/o root:motofit:reflectivity
			Newdatafolder/o root:motofit:reflectivity:SLDdatabase
		endif
		PauseUpdate; Silent 1		// building window...
		NewPanel /W=(0,0,520,320)
		Tabcontrol sldtab size = {511,305},tabLabel(0)="calculator",proc=Moto_SldtabControl,fSize=14
		Tabcontrol sldtab size = {511,305},tabLabel(1)="database",fSize=14
		Tabcontrol sldtab size = {511,305},tabLabel(2)="SLDmixing",fSize=14
		
		Button Addchemical,pos={20,50},size={110,30},proc=Moto_add_chemical,title="Add chemical"
		Button Addchemical,fSize=14
		PopupMenu listchemicals,pos={197,50},size={203,24},proc=Moto_updateSLDdisplay,title="List of chemicals"
		PopupMenu listchemicals,fSize=14
		PopupMenu listchemicals,mode=3,bodyWidth= 100,popvalue="_none_",value= #"root:motofit:reflectivity:SLDdatabase:chemicals"
		Button loaddatabase,pos={20,85},size={110,30},proc=Moto_Loaddatabase,title="load database"
		Button loaddatabase,fSize=14
		SetVariable neutronSLD,pos={220,90},size={180,19},title="Neutron SLD",fSize=14
		SetVariable neutronSLD,value= k0,bodyWidth= 100
		SetVariable XraySLD,pos={236,130},size={164,19},title="X-ray SLD",fSize=14
		SetVariable XraySLD,value= k0,bodyWidth= 100
		SetVariable rho,pos={217,170},size={183,19},title="Mass density",fSize=14
		...
		Button savedatabase,fSize=14
		SetVariable chemcom,pos={162,210},size={238,19},title="Chemical Composition"
		SetVariable chemcom,fSize=14,value= S_Value,bodyWidth= 100
		SetVariable chemcom,value= S_Value
		DoWindow/C/T SLDpanel,"SLDdatabase"
		
		SetVariable chemical,pos={22,31},size={331,23},title="Chemical Formula",fSize=14
		SetVariable chemical,limits={inf,inf,0},value= root:motofit:reflectivity:SLDdatabase:calcSLD_chemical,bodyWidth= 200,proc = Moto_SLDcalculateSetvariable
		SetVariable calcMASSDENSITY,pos={18,60},size={200,23},title="Mass density",fSize=14,proc=Moto_SLDcalculateSetvariable
		SetVariable calcMASSDENSITY,limits={0,100,0.02},value= root:motofit:reflectivity:SLDdatabase:calcSLD_massdensity
		SetVariable calcMolVol,pos={236,60},size={230,23},title="Molecular volume (A^3)",fSize=14,proc=Moto_SLDcalculateSetvariable
		SetVariable calcMolVol,limits={0,inf,1},value= root:motofit:reflectivity:SLDdatabase:calcSLD_molvol
		
		SetVariable calcNeutronSLD,pos={57,171},size={294,23},title="Neutron SLD",fSize=14
		SetVariable calcNeutronSLD,limits={-inf,inf,0},value= root:motofit:reflectivity:SLDdatabase:calcSLD_neutron,bodyWidth= 197
		SetVariable calcXRAYSLD,pos={82,208},size={269,23},title="Xray SLD",fSize=14
		SetVariable calcXRAYSLD,limits={-inf,inf,0},value= root:motofit:reflectivity:SLDdatabase:calcSLD_Xray,bodyWidth= 197
		Button CALCULATE,pos={194,100},size={100,60},title="Calculate",fSize=14,proc = Moto_SLDcalculateButton
		Button AddToDataBase,pos={194,235},size={100,60},title="Add to \r database",fSize=14,proc = Moto_addchemicalfromcalculator
		
		//make the variables for the mixing
		variable/g root:motofit:reflectivity:SLDdatabase:mixSLD1=6.36,root:motofit:reflectivity:SLDdatabase:mixSLD2=-0.56
		variable/g root:motofit:reflectivity:SLDdatabase:mixOverallSLD=6.36
		variable/g root:motofit:reflectivity:SLDdatabase:mixvolfrac1=1,root:motofit:reflectivity:SLDdatabase:mixvolfrac2=0
		NVAR/z mixOverallSLD = root:motofit:reflectivity:SLDdatabase:mixOverallSLD
				
		SetVariable mixSLD1,pos={29,57},size={200,23},title="SLD 1st component",fSize=14,value =root:motofit:reflectivity:SLDdatabase:mixSLD1
		Setvariable mixSLD1,proc = Moto_mixCalculateSetvariable,limits={-inf,inf,0.01}
		SetVariable mixSLD2,pos={243,57},size={200,23},title="SLD 2nd component",fSize=14,value=root:motofit:reflectivity:SLDdatabase:mixSLD2
		Setvariable mixSLD2, proc = Moto_mixCalculateSetvariable,limits={-inf,inf,0.01}
		SetVariable mixvolfrac1,pos={29,105},size={200,23},title="vol. frac. 1st component",fSize=14,value=root:motofit:reflectivity:SLDdatabase:mixvolfrac1
		Setvariable mixvolfrac1, limits={0,1,0.01},proc= Moto_mixCalculateSetvariable
		SetVariable mixvolfrac2,pos={243,105},size={200,23},title="vol. frac. 2nd component",fSize=14,value=root:motofit:reflectivity:SLDdatabase:mixvolfrac2
		Setvariable mixvolfrac2, limits={0,1,0},proc = Moto_mixCalculateSetvariable
		setvariable mixoverallSLD,pos={164,246},size={160,23},title="Overall SLD",fSize=14,value=mixoverallSLD,limits={-inf,inf,0.01},proc=Moto_mixCalculateSetvarReverse
...
Function Moto_SLDintocoef_Cref(ctrlName,popNum,popStr) : PopupMenuControl
	...
	Wave neutronsld=$"root:motofit:reflectivity:SLDdatabase:SLD_neutrons"
	Wave xraysld=$"root:motofit:reflectivity:SLDdatabase:SLD_X_rays"
...
Function Moto_localchi2() //menu call
	...
	duplicate/o parwave, root:motofit:reflectivity:tempwaves:tempparwave
	Wave tempparwave = root:motofit:reflectivity:tempwaves:tempparwave
	
	//do you want to examine a 1D or 2D chi2map
	String dim
	Prompt dim,"Do you want to examine a 1D or 2D or 3D chi2map?",popup,"1D;2D;3D"
	...
	strswitch (dim)
		...
		case "2D":
			...
			break
		case "3D":
			if(itemsinlist(operationlist("createslicer",";","external"))==0)
				Setdatafolder callfolder
				ABORT "please install the graphical slicer XOP"
			endif
			try
				controlinfo/w=reflectivitypanel useerrors
				if(V_Value & waveexists(ee))
					Moto_localchi2graph3D("motofit",tempparwave,yy,xx,ee)	
				else
					duplicate/o yy,temp_e
					temp_e=1
					Moto_localchi2graph3D("motofit",tempparwave,yy,xx,temp_e)
				endif
			catch
			endtry
			break
...
Function Moto_localchi2graph2D(funcname,parwave,yy,xx,ee)
	...
	Setdatafolder callfolder
End
Function Moto_localchi2graph3D(funcname,parwave,yy,xx,ee)
	String funcname
	wave parwave,yy,xx,ee
	Struct GEN_optimisation gen
	Wave gen.GEN_yy=yy
	...
	variable whichParam=1,whichparam2=2,whichparam3=3,explorelimit=5,explorelimit2=5,explorelimit3=5
	prompt whichParam,"which variable would you like to inspect?"
	prompt whichParam2,"which variable would you like to inspect?"
	prompt whichParam3,"which variable would you like to inspect?"
	prompt explorelimit,"what percentage either side would you like to explore?"
	prompt explorelimit2,"what percentage either side would you like to explore?"
	prompt explorelimit3,"what percentage either side would you like to explore?"
	Doprompt "lets inspect the Chi2 map for a parameter?",whichParam,whichParam2,whichparam3,explorelimit,explorelimit2,explorelimit3
	if(V_flag==1)
		abort
	endif
	//if the parameter is valid then abort
	...
	if(whichparam3>numpnts(parwave)-1 | whichparam3==0)
		abort "parameter chosen is outside the limits"
	endif
	...
	make/o/n=3 tempcoefs
	tempcoefs[0]=parwave[whichparam]
	tempcoefs[1]=parwave[whichparam2]
	tempcoefs[2]=parwave[whichparam3]
	Wave gen.GEN_pvector=tempcoefs
	...
	variable chi2,ii,value,increm,increment,increm2,increment2,jj,increm3,increment3,kk
	Make/o/d/n=(41,41,41) chi2map3D
	increm=0.01*explorelimit*gen.GEN_parwave[whichparam]
	increm2=0.01*explorelimit2*gen.GEN_parwave[whichparam2]
	increm3=0.01*explorelimit3*gen.GEN_parwave[whichparam3]
	increment=increm/((dimsize(chi2map3D,1)-1)/2)
	increment2=increm2/((dimsize(chi2map3D,0)-1)/2)
	increment3=increm3/((dimsize(chi2map3D,2)-1)/2)

	variable left=gen.GEN_parwave[whichparam]-increm,right=gen.GEN_parwave[whichparam]+increm
	variable left2=gen.GEN_parwave[whichparam2]-increm2,right2=gen.GEN_parwave[whichparam2]+increm2
	variable left3=gen.GEN_parwave[whichparam3]-increm3,right3=gen.GEN_parwave[whichparam3]+increm3
	setscale/p y,left,increment,chi2map3d
	setscale/p x,left2,increment2,chi2map3d
	setscale/p z,left3,increment3,chi2map3d
	
	for(jj=0;jj<(dimsize(chi2map3d,1));jj+=1)
		value=left+increment*jj
		gen.GEN_pvector[0]=value
		for(ii=0;ii<(dimsize(chi2map3d,0));ii+=1)
			value=left2+increment2*ii
			gen.GEN_pvector[1]=value
			for(kk=0;kk<(dimsize(chi2map3d,2));kk+=1)
				value = left3+increment3*kk
				gen.GEN_pvector[2]=value
				chi2map3d[ii][jj][kk]=GEN_optimise#GEN_chi2(gen)/numpnts(yy)
			endfor
		endfor
	endfor
	
	Moto_scaleimage(chi2map3D)
	string cmd = "createslicer"
	execute/q/z cmd
	cmd = "ModifySlicer srcWave=W_scaleimage;ModifySlicer NewWidthSlice = 0.5"
	execute/q/z cmd
	cmd = "ModifySlicer NewLengthSlice = 0.5;Modifyslicer Newheightslice = 0.5"
	execute/q/z cmd
	Setdatafolder callfolder
End

Function Moto_Findminmax3Dimage(w,minrow,mincol,minlay,maxrow,maxcol,maxlay)
	//this function finds the minimum and maximum positions of a 3D wave
	Wave w
	variable &minrow,&mincol,&minlay,&maxrow,&maxcol,&maxlay

	variable ii=0,vmin,vmax
	for(ii=0;ii<dimsize(w,2);ii+=1)
		imagestats/p=(ii) w
		if(ii==0)
			Vmin=V_min
			Vmax=V_max
...
Function Moto_scaleimage(w)
	//this function normalises an image wave to the minimum and maximum values in a 3D matrix.
	Wave w
	string saveDF=getdatafolder(1)
	Duplicate/o w, W_Scaleimage
	variable minrow,mincol,minlay,maxrow,maxcol,maxlay,vmin,vmax
	variable ii,jj,kk
	Moto_Findminmax3Dimage(w,minrow,mincol,minlay,maxrow,maxcol,maxlay)
	vmin=w[minrow][mincol][minlay]
	vmax=w[maxrow][maxcol][maxlay]
	
	for(ii=0;ii<dimsize(w,2);ii+=1)
		for(jj=0;jj<dimsize(w,1);jj+=1)
			for(kk=0;kk<dimsize(w,0);kk+=1)
				W_scaleimage[kk][jj][ii] = (w[kk][jj][ii] - vmin)/(vmax-vmin)				
			endfor
		endfor
	endfor
...
Function moto_modelchange(LB_Struct) : ListboxControl
	...
	String/G root:motofit:reflectivity:currentfocus  = "listwave:"+whichlist+";row:"+num2str(LB_Struct.row)+";col:"+num2str(LB_Struct.col)
	variable SLDval
	...
	if(LB_Struct.eventcode==4)		//enter SLD values into layer params
		if(LB_struct.eventmod & 2^4 && cmpstr(whichlist,"layerparams")==0 && LB_Struct.col==5)
			...
			ListBox layerparams,listWave=root:motofit:reflectivity:layerparams,win=reflectivitypanel
			ListBox layerparams,selWave=root:motofit:reflectivity:layerparams_selwave,win=reflectivitypanel
...
Function Moto_SLDselection()
	SVAR chemicals = root:motofit:reflectivity:SLDDatabase:chemicals
	...
	if(V_value==1) //neutrons
		Wave SLD =  root:motofit:Reflectivity:SLDdatabase:SLD_neutrons
	elseif(V_Value==2)
		Wave SLD = root:motofit:Reflectivity:SLDdatabase:SLD_X_Rays
	endif
...
Function Moto_varyparams(ctrlName) : ButtonControl
	String ctrlName
	Struct WMListboxAction LB_Struct
	
	SVAR/z currentfocus = root:motofit:reflectivity:currentfocus
	string localwavestr = stringbykey("listwave",currentfocus)
	Wave/T localwave = root:motofit:reflectivity:$localwavestr
	variable row = str2num(stringbykey("row",currentfocus))
	variable col = str2num(stringbykey("col",currentfocus))
	variable number = str2num(localwave[row][col])	
	
	if(cmpstr(ctrlname,"increase")==0)
		number=number+0.02*number
	endif
	if(cmpstr(ctrlname,"decrease")==0)
		number=number-0.02*number
	endif
	
	if(row==0 && cmpstr(stringbykey("listwave",currentfocus),"baselayerparams")==0)
		number = dimsize(root:motofit:reflectivity:layerparams,0)
		LB_Struct.row=0
		LB_Struct.col=1
		Wave/t LB_Struct.listwave=localwave
		if(cmpstr(ctrlname,"increase")==0)
			number=number+1
		endif
		if(cmpstr(ctrlname,"decrease")==0)
	...
	localwave[row][col] = num2str(number)
	LB_Struct.eventcode=7
	moto_modelchange(LB_Struct)
	currentfocus=ReplaceStringByKey("listwave",currentfocus,localwavestr)
	currentfocus=ReplaceStringByKey("col",currentfocus,num2str(col))
	currentfocus=ReplaceStringByKey("row",currentfocus,num2str(row))
End
...
Function Moto_AboutPanel()
	...
	DrawText 70,64,"@ Andrew Nelson, 2005-2006"
...
Function Moto_SLDLoadScatteringlengths()
	...
	newdatafolder/o/s root:motofit
	newdatafolder/o/s root:motofit:reflectivity
	newdatafolder/o/s root:motofit:reflectivity:Slddatabase
...
Function Moto_SLDcalculateButton(ctrlName) : ButtonControl
	String ctrlName
	SVAR chemical = root:motofit:reflectivity:SLDdatabase:calcSLD_chemical
	SVAR SLD_Neutron = root:motofit:reflectivity:SLDdatabase:calcSLD_neutron
	SVAR SLD_Xray = root:motofit:reflectivity:SLDdatabase:calcSLD_Xray
	NVAR SLD_massdensity = root:motofit:reflectivity:SLDdatabase:calcSLD_massdensity
	Variable/C sld
...
Function Moto_SLDcalculateSetvariable(SV_Struct) : Setvariablecontrol
	...
	if(SV_Struct.eventcode!=-1)
		...
		SVAR SLD_Xray = root:motofit:reflectivity:SLDdatabase:calcSLD_Xray
		NVAR SLD_massdensity = root:motofit:reflectivity:SLDdatabase:calcSLD_massdensity
		NVAR SLD_molvol = root:motofit:reflectivity:SLDdatabase:calcSLD_molvol
...
String element = ""
	...
	if(strlen(chemical)>0)
		do
			...
			posintable = Moto_FindTextEntryInWave(scatlengths,s1,0,0)
			if(posintable==-1)
...
Function Moto_FindTextEntryInWave(textwave,strtomatch,start,col)
	Wave/T textwave
	string strtomatch
	variable start,col
	variable ii,position=-1

	if(start<dimsize(textwave,0))
		for(ii=start;ii<dimsize(textwave,0);ii+=1)
			textwave[ii][col] = removeending(textwave[ii][col], " ")
			if(!cmpstr(strtomatch,textwave[ii][col]))
				position = ii
				break
			endif
		endfor
...
Function Moto_mixCalculateSetvariable(SV_Struct) : Setvariablecontrol
	...
	STRUCT WMSetVariableAction &SV_Struct
	if(SV_Struct.eventcode!=-1)
		NVAR mixOverallSLD = root:motofit:reflectivity:SLDdatabase:mixOverallSLD
		NVAR mixSLD1 = root:motofit:reflectivity:SLDdatabase:mixSLD1
		NVAR mixSLD2 = root:motofit:reflectivity:SLDdatabase:mixSLD2
		NVAR mixvolfrac1 = root:motofit:reflectivity:SLDdatabase:mixvolfrac1
		NVAR mixvolfrac2 = root:motofit:reflectivity:SLDdatabase:mixvolfrac2
...
Function Moto_mixCalculateSetvarReverse(SV_Struct) : Setvariablecontrol
	...
	Struct WMSetVariableAction &SV_struct
	NVAR/z mixSLD1 = root:motofit:reflectivity:SLDdatabase:mixSLD1
	NVAR/z mixSLD2 = root:motofit:reflectivity:SLDdatabase:mixSLD2
	NVAR/z mixOverallSLD = root:motofit:reflectivity:SLDdatabase:mixOverallSLD
	NVAR/z mixvolfrac1 = root:motofit:reflectivity:SLDdatabase:mixvolfrac1
	NVAR/z mixvolfrac2 = root:motofit:reflectivity:SLDdatabase:mixvolfrac2
	variable volfrac
	if(SV_Struct.eventcode!=-1)
...
Function/S Moto_layerwave2txtstr(numericwave)
	...
	string retStr=""
	Wave baseparams = root:motofit:reflectivity:baselayerparams
...
Function Moto_notebookoutput(notebookname,yWave,additionaloutputstr)
	...
	if(Waveexists($coefwaveStr))
		Moto_Chi2_print()
		NVAR/z chi2sq = root:motofit:reflectivity:chisq
...
Function Moto_backupModel()
	string coefwave = moto_coefficientfocus()
	duplicate/o root:$coefwave, root:motofit:reflectivity:tempwaves:coef_Cref_BAK
End
...
Function Moto_restoreModel()
	...
	if(!waveexists(w))
		abort "no initial guess to roll back to"
	endif
	duplicate/o root:motofit:reflectivity:tempwaves:coef_Cref_BAK, root:$coefwave
